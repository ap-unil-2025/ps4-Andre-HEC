...
def save_contacts_to_file(contacts, filename="contacts.json"):
    ...
    contact_file = save_to_json(contacts, filename)
    return contact_file
...
def append_contact_to_file(contact, filename="contacts.json"):
    ...
    new_contact_list = save_contacts_to_file(contacts, filename)
...
def backup_file(source_filename, backup_filename):
    ...
    if data is None:
        return False
    backup = save_to_json(data, backup_filename)
    return backup
...
def merge_json_files(file1, file2, output_file):
    ...
    result = save_to_json(merged_data, output_file)
    return result
